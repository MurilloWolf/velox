...
import { useSponsorsMessages } from "@/i18n/hooks/useSponsorsMessages";
...
export default function SponsorsPageClient() {
  const { hero, differentiatorsSection, ctaCard, contact, mediaKit } =
    useSponsorsMessages();
  ...
  }, [contact.whatsappMessage, contact.whatsappNumber]);

  const contactHref = hero.primaryCta.link ?? whatsappUrl;
  const ctaCardHref = ctaCard.cta.link ?? contactHref;

  const differentiatorItems = useMemo(
    () =>
      differentiatorsSection.cards.map((card) => ({
        ...card,
        icon: differentiatorIconMap[card.icon],
      })),
    [differentiatorsSection.cards]
  );

  const handleMediaKitClick = useCallback(async () => {
    if (hero.secondaryCta.link) {
      window.open(hero.secondaryCta.link, "_blank", "noopener,noreferrer");
      return;
    }

    if (!mediaKit.fileId) {
      console.warn("Media kit file id not configured.");
      return;
    }
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={ctaCard.cta.ariaLabel}
                  >
